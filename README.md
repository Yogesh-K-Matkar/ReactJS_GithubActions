...
#### Push VS Code(Local Repo) -->> GitHub(Remote Repo):-

   - Create local project in VS Code using terminal cmd or open existing project folder in VS Code.
   ...
   - Install git external installer from [official website](https://git-scm.com/) 
      ...
      1. To check git is properly installed or not, in VS Code terminal try below cmd
       ...
       If the git cmd issues, then check the  git version using cmd.
        ...
        ```
       If again try to initialize git using above cmd and it works fine.

   - To verify the git is added properly in Local Repo using below cmd
  
        Syntax:-
     
        ```TERMINAL

            ls -a       (To list all including hidden files and folder in local repo, then check .git folder created or not)      
   ...
   - In VS Code link Github Repository to Local Repository using below cmd's
  
        Syntax:-
     
        ```TERMINAL

            git remote add origin <githhub-repository-url>      
        
        ```

   - To verify the git is properly enabled in VS Code
        ...
        ```
   - To verify branches, if required rename in VS Code
  
        Syntax:-
     
        ```TERMINAL

            git branch        (To check current branches in remote repository)      

            git branch -M <rename-branch-name>   (To rename current branch to <rename-branch-name>)
        
        ```
    - Then push local repository to remote GitHub repository using below cmd

        Syntax:-
     
        ```TERMINAL

            git push -u origin <current-branch-name>      (To push local repository to remote repository)
        
        ```

        When -u is used in cmd, then further push cmd will be automatically pushed to remote repository without mentioning origin and branch name.

        Syntax:-
     
        ```TERMINAL

            git push  (To push local repository to remote repository with -u used previously)
        
        ```

    - Then follow step as per changes in Local repository to sync with Remote repository.

        Syntax:-
     
        ```TERMINAL

            git add .   (To stage all changes in local repository)

            git commit -m "commit comments"    (To commit changes in local repository)

            git push origin main   (To push local repository to remote repository)
        
        ```

***

#### Pull GitHub(Remote Repo) -->> VS Code(Local Repo):-

   - In Github, first login to your GitHub account, then in the  tab Repositories, click on the New button & create a  repository by giving a Repository Name
  
         Syntax:-
      
            Yogesh_Matkar/<repository name> then click Create Repository button.

   - In VS Code, open terminal and move to the folder where you want to clone the repository using cd command.
        
       Syntax:-
        
       ```TERMINAL
    
                git clone <repository-url>      (To clone remote repository to local repository)
...
####  Git Branch Commands:-
   ...
   - To view all branches in Local Repo

       Syntax:-
     
       ```TERMINAL

            git branch      (To view all branches in local repository)        

       ```

   - To view current branch in Local Repo
  
       Syntax:-
        
       ```TERMINAL
    
                git branch --show-current      (To view current branch in local repository)        
    
       ```
   - To come out of current branch in Local Repo
       
       Syntax:-
      
       ```TERMINAL    
    
                git checkout <current branch name>      (To come out of current branch to main branch in local repository)        

                git checkout -b <new branch name>      (To come out of current branch and create new branch in local repository)        
    
       ```

   - To create a new branch in Local Repo

       Syntax:-
     
       ```TERMINAL

            git branch <branch-name>      (To create new branch in local repository)        

       ```   

   - To delete a current branch in Local Repo

       Syntax:-
     
       ```TERMINAL

            git branch -d <branch-name>      (To delete branch in local repository)        
...
##### Using Git Commands:-
   ...
   - To  check difference merge a branch in Local Repo to main branch

       Syntax:-
     
       ```TERMINAL
            git diff <branch-name>      (<branch-name> is name of branch in which you want to merge current branch features before that check code difference in local repository)        

            git merge <branch-name>      (<branch-name> is name of branch in which you want to merge current branch features in local repository)

    - **Merge Case**:-

       1. **Conflicts found**,

       - Then resolve the conflicts in the code editor.

       - Then click on the Merge Pull Request button is displayed.Click Merge Pull Request button to merge the changes into the main branch. Click Confirm Merge button to confirm the merge.

##### Using Github :-
   
   - Go to GitHub account-->Select the repository, and click on the Branches button, then select the branch that you want to merge into the main branch.

   - Then click on the New Pull Request button.

   - Then review the changes and click on the Create Pull Request button.
   ...
   - **Merge Case**:-
       ...
       - After merging the branch in Githhub, then pull the changes to local repository using below cmd

           Syntax:-
         
           ```TERMINAL

                git pull origin main      (To pull changes from remote repository to local repository)        
...
##### Using Git Commands:-
   ...
   - Reset after Staged in Local Repo

       Syntax:-
     
       ```TERMINAL

            git reset <file-name>      (To reset last staged in local repository in specific staged file)

            git reset     (To reset last staged in local repository from all staged files)        

       ```  
   - Reset after Commit in Local Repo
     ...
     - **View Commit History**:-
       ...
       ```TERMINAL
            ...
            git log      (To view all commit history in local repository)        
     ...
     - **Latest Commit**:-
       ...
       ```TERMINAL
            ...
            git reset --hard HEAD~1      (To reset latest last commit denoted by HEAD~1 changes in local repository from all staged files)        
     ...
     - **Specific Commit**:-    
       ...
       ```TERMINAL
            ...
            git reset <commit-hash>      (To reset specific commit changes in local repository)

            git reset --hard <commit-hash>    (To reset enitre all commit changes in local repository)        
...
- **CSS Style** - 
    ...
    - **Element Design Structure**
        ...
        - **Flexbox** is designed for one-dimensional layouts, aligning items either in a row or a column at a time. It works from the content outwards, ideal when you want to space items evenly in a single direction and let the content size determine spacing. Flexbox containers manage items along either a horizontal or vertical axis but not both simultaneously.
         ...
         Mainly used for navbar,menus and toolbar display.
